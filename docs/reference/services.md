...
`deployment/prod/.env` is the single secret file the operator fills in (template at `.env.example`). Both the JWT signing keypair and the Mongo rs0 keyfile are materialised automatically on first boot by one-shot init services (`jwt-keys-init`, `mongo-keyfile-init`) into docker-managed volumes — `docker compose up -d` works from a clean checkout with no host-side keygen. `scripts/gen-jwt-keys.sh` and `scripts/gen-mongo-keyfile.sh` are kept as manual escape hatches — see `docs/deployment.md`.
